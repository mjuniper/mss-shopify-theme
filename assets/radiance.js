...
var RADIANCE = {
  ...
  templateProduct : {
    init: function(){
      $('#quantity-container-noscript').hide();
      $('#quantity-container-js').show();
...
$.fn.balanceRowHeight = function(numPerRow) {
  ...
  for( var row = 1; row <= nRows; row++ ){
    ...
    $(this).find('li').slice(min, max).each(function(){
      ...
      //mjuniper: they were getting the tallestTitleInRow var but not doing anything with it (not sure thtat the :first is necessary)
      //I added applying it to the .product-information div below
...
function addToCart(e){
  ...
  var intQty = +$(this).parents('form').find('#integral-quantity').val();
  var fractQty = +$(this).parents('form').find('#fractional-quantity').val();
  var quantity = intQty + fractQty;
  
  //if (quantity < 2){
      //alert('Sorry, we have a minimum quantity of 1/2 yard for online orders.');      
      //return;
  //}
...
function addToCartFail(jqXHR, textStatus, errorThrown){
  ...
  if (json_product && json_product.type === 'fabric') {            
      if (msg.indexOf('We do not have ') > -1) {                  
          var re = /\d+\b/;
          var msgParts = msg.split(re);          
          var qtyIdx = msg.match(re);
          var qty = 0;
          msg = 'We do not have that much left.';
          if (qtyIdx && qtyIdx.length) {
              qty = qtyIdx[0];
              var frac = (qty % 4) / 4;
              ...
              msg = msgParts[0] + qty + frac + ' yards' + msgParts[1].substr(6);
          }  
      }
  }
  
  $('#add-to-cart-msg').addClass('error').text(msg).fadeIn('fast');
}
...
function updateCartDesc(data){
  ...
  switch(data.items.length){
    ...
    case 1:
      ...
      break;
    default:
      $cartLinkText.text(data.items.length+' items');
      break;
  }
}
